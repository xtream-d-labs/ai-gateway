// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// PostNewSessionParamsBody AccountInfo
// swagger:model postNewSessionParamsBody
type PostNewSessionParamsBody struct {

	// Password for the private Docker registry
	// Required: true
	// Format: password
	DockerPassword *strfmt.Password `json:"docker_password"`

	// Username for the private Docker registry
	// Required: true
	DockerUsername *string `json:"docker_username"`
}

// Validate validates this post new session params body
func (m *PostNewSessionParamsBody) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateDockerPassword(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateDockerUsername(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *PostNewSessionParamsBody) validateDockerPassword(formats strfmt.Registry) error {

	if err := validate.Required("docker_password", "body", m.DockerPassword); err != nil {
		return err
	}

	if err := validate.FormatOf("docker_password", "body", "password", m.DockerPassword.String(), formats); err != nil {
		return err
	}

	return nil
}

func (m *PostNewSessionParamsBody) validateDockerUsername(formats strfmt.Registry) error {

	if err := validate.Required("docker_username", "body", m.DockerUsername); err != nil {
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *PostNewSessionParamsBody) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *PostNewSessionParamsBody) UnmarshalBinary(b []byte) error {
	var res PostNewSessionParamsBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
