...
func (o *GetJobFilesURL) Build() (*url.URL, error) {
	...
	var _path = "/jobs/{id}/files"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("ID is required on GetJobFilesURL")
	}
...
func (o *GetJobFilesURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetJobFilesURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetJobFilesURL")
	}
