...
func (o *GetNotebookDetailsURL) Build() (*url.URL, error) {
	...
	var _path = "/notebooks/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("ID is required on GetNotebookDetailsURL")
	}
...
func (o *GetNotebookDetailsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetNotebookDetailsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetNotebookDetailsURL")
	}
